...
def mahrt_pan(
    ...
    **kwargs
    ...
    soil_parameters = {k: jnp.array(v,float) for k,v in soil_parameters.items()
                        if k not in ['Label', 'HSG']}

    # Set the default parameter configuration, including soil parameters
    configuration = soil_parameters | {
        ...
        'runoff'  : 1,     # If 1, runoff is assumed to leave the location;
    } | kwargs             # if 0 it is added to rainfall at each timestep.

    # Core function returns the last timestep values, and the full timeseries
    (layer_1, layer_2, surplus), timeseries = _jit_wrapper(
        evapotranspiration, precipitation, configuration, axis=axis)

    labels = ['layer_1', 'layer_2', 'surplus']
    return dict(zip(labels, timeseries))


@partial(jax.jit, static_argnames='axis')
def _jit_wrapper(
    evapotranspiration : jnp.ndarray, 
    precipitation      : jnp.ndarray, 
    configuration      : dict,
    axis               : int,
):
    """ JIT compile the whole function now that we've parsed the inputs """
    ...
    def calculate_timestep(evapot, precip,
        ...
        gt_0 = lambda x: jnp.clip(x, 0, None)
        lt_1 = lambda x: jnp.clip(x, None, 0.999)
        in01 = lambda x: jnp.clip(x, 0.001, 0.999)
        ...
        def runoff(precip, cn, sw_frac, Smin, Smax):
            ...
            def scn_adjust(cn, sw_frac, Smin, Smax):
                ...
                inner = ~(lower | upper)
                cn100 = cn / 100
                return jnp.where(cn == 100, cn, 100 * (     (cn100 * inner) +
        ...
        thetaM = jnp.maximum(theta1, theta2)
        ...
        delta = jnp.clip(G1.diffuse*d_coefs + G1.conduct*k_coefs, -limit,limit)
        ...
        return (jnp.clip(theta1, Smin, Smax),
                jnp.clip(theta2, Smin, Smax),
                surplus)

    # Convert to JAX arrays
    evapotranspiration = jnp.asarray(evapotranspiration)
    precipitation = jnp.asarray(precipitation)
    C = configuration

    # Allow equation parameters to also be dynamic in time (which assumes
    # the time axis is the same as for precipitation)
    uses_dt = lambda v: jnp.shape(v)[:axis+1] == precipitation.shape[:axis+1]
    dynamic = {k: jnp.asarray(C.pop(k)) for k in list(C) if uses_dt(C[k])}

    # Get the expected dimension sizes if there are multiple dimensions
    dimsize = [size for d, size in enumerate(precipitation.shape) if d != axis]

    def broadcast(v):
        """ Add extra dimensions as necessary to allow array broadcasting """
        if jnp.size(v) > 1:
            assert(jnp.shape(v)[0] == dimsize[0]), [jnp.shape(v), dimsize]
            return jnp.reshape(v, v.shape+(1,)*(len(dimsize)-jnp.ndim(v)))
        return v
    C = {k: broadcast(v) for k,v in C.items()}

    # Ensure the carried state over the timestep loop has the appropriate shape
    tiled = lambda v: v if jnp.size(v)>1 else jnp.tile(v, dimsize)
    carry = (
        jnp.atleast_1d(tiled( C.pop('theta1') )),
        jnp.atleast_1d(tiled( C.pop('theta2') )),
        jnp.atleast_1d(tiled( C['surface'] )),
    )

    # Include any dynamic parameters in the tuple of inputs to loop over
    inputs = (evapotranspiration, precipitation)
    inputs = inputs + tuple(dynamic[k] for k in dynamic)
    inputs = tuple(jnp.moveaxis(v, axis, 0) for v in inputs)

    @jax.jit
    def scan(carry, step_data):
        evap, precip, *dynamics = step_data
        layer1, layer2, surplus = carry
        precip_w_runoff_surplus = precip + (surplus * (1-C['runoff']))

        carry = calculate_timestep(evap, precip_w_runoff_surplus, 
            theta1=layer1, theta2=layer2,
            **(C | dict(zip(dynamic, map(broadcast, dynamics)))))
        return carry, jnp.stack(carry, axis=0)
    
    # Scan over all timesteps
    last_step, timeseries = jax.lax.scan(scan, carry, inputs)

    # Remove the final batch dimension if the input only has a temporal dim
    if jnp.ndim(evapotranspiration) == 1:
        timeseries = jnp.squeeze(timeseries, axis=-1)

    # Shift the temporal dimension back into its original location
    return last_step, jnp.moveaxis(timeseries, 0, axis+1)